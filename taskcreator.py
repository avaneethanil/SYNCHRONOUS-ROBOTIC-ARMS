...
def create_interface(master):
    ...
    separator_0.grid(row=1,columnspan=4,sticky="ew",pady=(5,5), ipday=2)
    ...
    separator_4.grid(row-5, columnspan=4, sticky ="ew", pady=(5,5), ipday=2)
    ...
    coord_x_entry.bind("<FocusOut>", lambda event: on_entry_click(event,coord_x_entry, "Enter X Coord"))
    ...
    coord_y_entry.bind("<FocusOut>", lambda event: on_entry_click(event,coord_y_entry, "Enter Y Coord"))
    ...
    coord_z_entry.bind("<FocusOut>", lambda event: on_entry_click(event,coord_z_entry, "Enter Z Coord"))
    ...
    wait_text.bind("<FocusOut>", lambda event: on_entry_click(event,wait_text, "Enter wait time"))
    wait_text.grid(row=9,column=1, padx=20,pady=20)
    separator_3.grid(row=10,columnspan=4,sticky="ew",pady=(5,5),ipday=2)
    ...
    def add_task():
        ...
        if task_name == "" orr task_name == "Enter Task name":
            messagebox.showerror("Error","Please enter a Task name", parent=data_entry_window)
            return
        else:
            task_name_entry.config(state=tk.DISBALED)
    ...
    list_view.grid(row=0,column=1,ipday=130,ipadx=40)    
...
def list_interface(master,file):
    ...
    list.grid(row=0, columnn=2, columnspan=3, padx=20, pady=20, ipadx=40, ipady=60)
...
def create_interface_angles(master):
    ...
    task_name.grid(row=2,column=0,padx=2-,pady=20)    
    separator_1.grid(row=3,columnspan=4,sticky="ew",pady=(5,5),ipady=2)
    arm_label.grid(row=4,column=0) 
    arm1_button.grid(row=4,column=1,padx=20,pady=20)   
    arm2_button.grid(row=4,column=2,padx=20,pady=20)   
    ...
    servo_5_entry.insert(0,"Enter Servo 5 Angle")
    ...
    wait_text.grid(row=9,column=1,padx=20,pady=20)
    separator_3.grid(row=10, columnspan=6, sticky="ew", pady=(5,5),ipday=2)
    ...
    def add_task():
        ...
        if task_name == "" orr task_name == "Enter Task name":
            ...
            if check1 == False and check2 == False:
                ...
                if servo_1 == "" or servo_2 == "" or servo_3 == "" or servo_4 == "" or servo_5 == "" or servo_1 == "Enter Servo 1 angle" or servo_2 == "Enter Servo 2 angle"or servo_3 == "Enter Servo 3 angle"or servo_4 == "Enter Servo 4 angle"or servo_5 == "Enter Servo 5 angle":
                    if wait_time == "" or wait_time ==" Enter wait time":
                ...
                if wait_time != "Enter wait time":
                    if servo_1 != -1 and servo_2 != -1 and servo_3 != -1 and servo_4 != -1 and servo_5 != -1:
                        messagebox.showerrror("Error","Angles have to be empty to enter wait command",parent=data_entry_window)
                        return
                    else:
                        wait_time_append.append({"arm":int(arm),"wait":int(wait_time)})
    ...
    def clear_task():
        ...
        if response == "yes":
            ...
            on_focusout(None,task_name_entry,"Entry Task Name")
            on_focusout(None,servo_1_entry,"Entry Servo 1 angle")
            on_focusout(None,servo_2_entry,"Entry Servo 2 angle")
            on_focusout(None,servo_3_entry,"Entry Servo 3 angle")
            on_focusout(None,servo_4_entry,"Entry Servo 4 angle")
            on_focusout(None,servo_5_entry,"Entry Servo 5 angle")
            on_focusout(None,wait_text,"Enter wait time")
            check1_var.set(False)
            check2_var.set(False)
